import fetchAssetFromSignedUrl from '../functions/fetchAssetFromSignedUrl';
...
describe('fetchAssetFromSignedUrl', () => {
  ...
  it('should fetch a url and return a File object', async () => {
    ...
    const file = await fetchAssetFromSignedUrl({ url: mockUrl, fileName: mockFileName });
  ...
  it('should throw an error if fetch fails', async () => {
    ...
    await expect(fetchAssetFromSignedUrl({ url: mockUrl, fileName: mockFileName })).rejects.toThrow(
      `Failed to fetch file from URL: ${mockUrl}`
    );
  });
});